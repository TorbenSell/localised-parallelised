...
try:
    ...
    ''' Initialise exponential Blurring kernel '''
    H = np.zeros((m**2,m**2))
    for j in range(m):
        for i in range(m):
            for l in range(m):
                for k in range(m):
                    ''' For the torus: '''
                    d = np.sqrt(min(abs(k-i),m-abs(k-i))**2+min(abs(l-j),m-abs(l-j))**2)
                    H[i+j*m,k+l*m] = np.exp(-(d**2)/2/(0.7**2))/(2*np.pi*0.7**2)
    np.save('ID/H.npy',H)
...
#plt.savefig('ID/img_axobs.png', bbox_inches='tight')